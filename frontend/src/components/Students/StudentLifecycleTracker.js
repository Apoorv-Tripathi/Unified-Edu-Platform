...
const StudentLifecycleTracker = ({ studentId, studentData }) => {
  ...
  const loadLifecycle = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/students/${studentId}/lifecycle`, {
        headers: {
  ...
  const handleSaveStage = async () => {
    ...
    try {
      ...
      const url = showEditModal
        ? `http://localhost:5001/api/students/${studentId}/lifecycle/${selectedStage._id}`
        : `http://localhost:5001/api/students/${studentId}/lifecycle`;
