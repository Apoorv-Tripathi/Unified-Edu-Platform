...
// Generate APAAR ID safely (no duplicates)
studentSchema.pre('save', function (next) {
  if (!this.apaarId || this.apaarId.trim() === "") {
    const year = new Date().getFullYear();
    const random = crypto.randomBytes(4).toString('hex').toUpperCase(); // 8-char random
    this.apaarId = `APAAR-${year}-${random}`;
  }
  next();
});
